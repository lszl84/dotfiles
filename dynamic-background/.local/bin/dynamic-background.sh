...
while true; do
  for i in {1..30}; do
    ...
    convert ~/.config/labwc/fedora-bw.png   -font "DejaVu-Sans-Mono" -pointsize 32 -fill '#cccccc'   -annotate +$(( $(identify -format "%w" ~/.config/labwc/fedora-bw.png) / 2 +60))+780 "$(cat /tmp/ff.txt)"   ~/.config/fastfetch_wallpaper.png && rm /tmp/ff.txt
    swaybg -i ~/.config/fastfetch_wallpaper.png -m center &
    my_swaypid_new=$!
    sleep 60
