...
Refreshes every 30 seconds via =dwm-status=. Instant volume updates via
...
- dwm font set to =monospace:size=10=, border to 4px, gaps to 14px (7 logical)
...
Settings are applied via =dwm-touchpad=, a background daemon launched from =dwm-start=:

- *Natural scrolling*: enabled
- *Tap-to-click*: disabled
- *Click method*: clickfinger (no button areas)

The daemon uses =xinput= to set libinput properties on all pointing devices,
and re-applies them automatically when:

1. *Devices are added* — monitored via =udevadm monitor= (covers power-management
   re-enumeration, USB hotplug, etc.)
2. *System resumes from suspend* — monitored via DBus =PrepareForSleep= signal

This avoids needing root or =/etc/X11/xorg.conf.d/= files.
...
- *Transparency* (90% opacity) on Emacs and Xfce4-terminal windows
...
# Stow the configs
cd ~/dotfiles && stow dwm touchegg picom
...
| =dwm-touchpad=      | Touchpad settings daemon (survive resume)  |
