...
#+DESCRIPTION: dwm setup for Arch Linux with HiDPI
...
Uses a built-in dwm locker (PAM auth, OverrideRedirect black window) triggered
via =xss-lock= for IdleHint and sleep lock transfer:

1. =xset s 180 0= sets the X screensaver timeout (or =Super+l= / =loginctl lock-session=)
2. =xss-lock= launches =dwm-lock=, which sets =_DWM_LOCK= root property
3. dwm's =propertynotify()= detects this and calls =lock()=
4. =lock()= creates a fullscreen black window, grabs keyboard/pointer, collects password
5. PAM authentication runs in a forked child (pipe-based result, safe with =SA_NOCLDWAIT=)
6. On unlock, dwm deletes =_DWM_LOCK= property; =dwm-lock= sees this and exits
7. =xss-lock= sets logind =IdleHint=true=; after =IdleActionSec=, logind suspends

The built-in locker avoids the VT switch flash that external lockers have — the
black window is painted in-process before any frame of desktop content is visible.

Requires =/etc/pam.d/dwm= with =@include common-auth= and =@include common-account=.

Configured in:
- =dwm-start=: =xset s 180 0= (screensaver timeout)
- =/etc/systemd/logind.conf=: =IdleAction=suspend=, =IdleActionSec=2min=
...
- *dwm-lock* (=dwm-lock-patch.py=): built-in PAM screen locker
...
sudo pacman -S xss-lock brightnessctl rofi xsettingsd picom \
    libx11 libxft libxinerama harfbuzz pam xorg-xsetroot feh touchegg polkit-gnome
...
| =dwm-lock=          | xss-lock wrapper (signals dwm to lock)     |
