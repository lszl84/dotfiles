...
Uses =xss-lock= with =--transfer-sleep-lock= protocol, =xsecurelock=, and
systemd =IdleAction= for idle suspend:

1. =xset s 600 0= sets a 10-minute X screensaver timeout
2. When the screensaver activates, =xss-lock= launches =xsecurelock=
3. =xsecurelock= handles the sleep lock fd transfer natively
4. =xss-lock= also sets logind =IdleHint=true= for the session
5. After all sessions are idle for =IdleActionSec= (15 min), logind suspends
6. On wake, =xsecurelock= is still running — password required

Total: 10 min idle → lock, 25 min idle → suspend. Configured in:
- =dwm-start=: =xset s 600 0= (screensaver timeout)
- =/etc/systemd/logind.conf=: =IdleAction=suspend=, =IdleActionSec=15min=
...
sudo apt install xss-lock xsecurelock brightnessctl rofi xsettingsd picom \
